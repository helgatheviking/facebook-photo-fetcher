 ...
 */
add_action('save_post', 'fpf_run_main');
...
/**
  ...
  *                     //For a full list of what's available see fpf_fetch_album_content().
  */
function fpf_find_shortcode($post_content){
    //Start by splitting the content at startTag, and check for "none" or "too many" occurrences
    global $fpf_shortcode_tag;
    ...
    // convert to existing params for fetching images
    if( isset( $atts[0] ) ) {
        foreach( $atts[0] as $att ){
            $pieces = explode("=", $att);
            if( count( $pieces ) == 2 ){
                $params[$pieces[0]] = str_replace ( '"' , '' , $pieces[1] );
            } 
        }
    }

    return apply_filters('fpf_parse_shortcode_params', wp_parse_args( $params, fpf_default_albumparams() ) );
}

/**
  * Album defaults 
  */
function fpf_default_albumparams(){
    return apply_filters( 'fpf_default_albumparams', array( 
...
function fpf_fetch_album_content($aid, $params)
{
    ...
    $params = wp_parse_args( $params, fpf_default_albumparams() );
    ...
    {    
        ...
        $photos = array_slice($photos, intval($params['start']), intval($params['max'])); 
